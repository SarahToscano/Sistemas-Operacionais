...
def main():
    ...
    faltas_lru = LRU(paginas, quadros, table, seq4)
...
def procura(seq, direcao, table,quadros,historico):
    ant=0; aux=[0]*quadros
    agr=0;j=0;nada=0
    indice=0
    melhor_op=-1
    ...
    if(direcao==0): #OTM
        ...
        for l in range(0,quadros):
            if(aux[l]==0):
                print("Sua melhor opção eh o valor:", seq[l], "que esta no indice:", l)
                return(l)
        for i in range (0, len(seq)):
            if((seq[i] in table)&(len(seq)>2)): # Achamos uma pagina listada e nao eh a ultima
            ...
            if((i==len(seq)-1)): #Acabaram as paginas e nenhuma foi encontrada = FIFO
               if(melhor_op==-1):
                   if(j==quadros-1):
                       ...
                       #print("Sua melhor opção eh o valor:", melhor_op, "que esta no indice:", indice)
                   j+=1
                   print("Fudeu,Acabaram as paginas e nenhuma foi encontrada fazer FIFO")
               else:
...
def OTM(paginas,quadros, table,seq3):
    faltas = 0; count=0; aux=0;i=0;indice=0
    historico=[]
    print("Faltas=",faltas,"\n")
    if (paginas >= quadros):
        print("seq2:", seq3)
            ...
            while (len(seq3) > 0):
                ...
                while(i<quadros):
                    ...
                    if(seq3[i] in table):
                        ...
                        print("tabela", table)
                        print("sequencia nova:", seq3, "\n")
                        indice=procura(seq3, 0, table,quadros,historico)
                        print("\nVoltei p funcao")
                        print("table",  table)
                        print("seq", seq3)
...
def LRU(paginas, quadros, table, seq4):
    print(table)
    faltas=0;i=0;historico=[]
    if (paginas >= quadros):
        print("num de faltas:", faltas, "\n")
        while (-1 in table):
            while (i < quadros):  # tipo um for
                if (not (seq4[0] in table)):
            ...
            while (len(seq4) > 0):
                ...
                while (i < quadros):
                    ...
                    if (seq4[i] in table):
                        ...
                        #indice = procura(seq4, 1, table, quadros)
                        print("\nVoltei p funcao")
                        print("table", table)
                        print("seq", seq4)
